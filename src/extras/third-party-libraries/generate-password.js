/*
By: Brendan Ashworth @brendanashworth
Link: https://github.com/brendanashworth/generate-password
Modif: 26/12/2020 *crypto not work in react native;
*/
...
var getNextRandomValue = function () {
    if (randomIndex === undefined || randomIndex >= randomBytes.length) {
        ...
        const randomArray = [];
        while (randomArray.length < 256) {
            randomArray.push(Math.ceil(Math.random() * 256));
        }
        randomBytes = Buffer.from(randomArray)// crypto.randomBytes(RANDOM_BATCH_SIZE);
    }
